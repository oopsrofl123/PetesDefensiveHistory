...
function updateAnchors(slot)
    if UnitExists(slot) then
        local partyframe = slotToPartyFrame(slot)
        if partyframe then
            local cdb = partyframe.CenterDefensiveBuff

			historyRows[slot]:SetPoint("RIGHT", slotToPartyFrame(slot), "LEFT", -2, 0)
            ...
            cdb.historyRow = historyRows[slot]
            ...
            cdb:SetScript("OnHide", function(frame)
                frame.historyRow:SetPoint("RIGHT", slotToPartyFrame(frame.historyRow.slot), "LEFT", -2, 0)
            end)
        end
    end
end
...
function isAbilityIdentified(item)
...
function findAuraIndex(row, auraName)
...
function showHistoryItemTracker(item)
    item.inferredCD:SetText(item.cooldown)
    if isAbilityIdentified(item) then
...
function addCompletedBuffToHistory(slot, buff)
    buff.duration = buff.endTime - buff.startTime
    printDebug("aura instance ID " .. buff.auraInstanceID .. " removed from " .. slot .. " after " .. buff.duration .. " seconds")
    ...
    slot = identifyAbility(slot, buff)
    ...
    if isAbilityIdentified(buff) then
        local index = findAuraIndex(row, buff.auraName)
        if index then
    ...
    item = row.historyItems[MAX_HISTORY]
    ...
    -- Blizzard's texture.
    if isAbilityIdentified(buff) and buff.iconId then
...
function shiftHistoryLeftFrom(historyItems, fromIndex)
...
-- reasonable behavior.
function shiftHistoryRightFrom(historyItems, fromIndex)
...
function shiftHistoryItem(from, to)
    ...
    showIfShown(from, to)
    showIfShown(from.timer, to.timer)
    showIfShown(from.icon, to.icon)
    -- swipe texture animations have to be restarted in addition to preserving
    -- visibility.
    showIfShown(from.swipeTexture, to.swipeTexture)
    ...
    showIfShown(from.inferredCD, to.inferredCD)
end
...
function clearHistoryItem(item)
    ...
    showDebugVisual(item.icon)  -- show empty icons in debug mode
    ...
    showDebugVisual(item.inferredCD)
...
function allocHistoryItem(row, slot, index)
    local frameName = "PetesDefensiveHistory_" .. slot .. "_" .. index
...
local function allocRow(slot)
    local row = CreateFrame("Frame", "PetesDefensiveHistoryRow" .. slot, UIParent)

    row.slot = slot
    historyRows[slot] = row
    ...
    row:SetSize(MAX_HISTORY*ICON_SIZE + (MAX_HISTORY-1)*ICON_SPACING, ICON_SIZE)
    row:SetPoint("RIGHT", slotToPartyFrame(slot), "LEFT", -2, 0)
    ...
    for i=1,MAX_HISTORY do
        ...
        historyItem:SetPoint("LEFT", row, "LEFT", (i-1)*(ICON_SIZE+ICON_SPACING), 0)
...
function updateRow(slot, specId, playerName)
...
-- to /reload.
function allocHistoryGrid()
    if pdhInitialized then
        printDebug("allocHistoryGrid() called after already being initialized")
        return
    end
    ...
    for slot, _ in pairs(allSlots) do
        historyRows[slot] = allocRow(slot)
    end
...
function pdhReset()
    ...
    for slot, row in pairs(historyRows) do
        clearRow(row)
        updateAnchors(slot)
        if UnitExists(slot) then
            printDebug("showing row for slot " .. slot)
            showRow(slot)
        else
            printDebug("hiding row for slot " .. slot)
        end
    end
